...
class HomeController extends GetxController {
  ...
  Future refreshHome({bool showMessage = false, String statusId}) async {
    ...
    await getStatistics();
    Get.find.getNotificationsCount();
    changeTab(statusId);
    if (showMessage) {
      Get.showSnackbar(
  ...
  Future<void> acceptBookingService(Booking booking) async {
    final _status = Get.find.getStatusByOrder(Get.find.global.value.accepted);
  ...
  Future<void> declineBookingService(Booking booking) async {
    try {
      if (booking.status.order < Get.find.global.value.onTheWay) {
        final _status = getStatusByOrder(Get.find.global.value.failed);
        final _booking =
