...
class LaravelApiClient extends GetxService with ApiClient {
  ...
  Future<User> loginSocial(Map<String, dynamic> user) async {
    Uri _uri = getApiBaseUri("provider/login-social");
    printUri(StackTrace.current, _uri);
    var response = await _httpClient.postUri(
      _uri,
  ...
  // Future<List<EService>> getEServices() async{
  //   if (!authService.isAuth) {
  //     throw new Exception("You don't have the permission to access to this area!".tr + "[ getEServices() ]");
  //   }
  //   var _queryParameters = {
  //     'only': 'id;name',
  //     'orderBy': 'created_at',
